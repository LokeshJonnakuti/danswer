...
class EmbeddingModel:
    ...
    def encode(self, texts: list[str], text_type: EmbedTextType) -> list[list[float]]:
        ...
        response = requests.post(
            self.embed_server_endpoint, json=embed_request.dict(), timeout=60
        )
...
class CrossEncoderEnsembleModel:
    ...
    def predict(self, query: str, passages: list[str]) -> list[list[float]]:
        ...
        response = requests.post(
            self.rerank_server_endpoint, json=rerank_request.dict(), timeout=60
        )
        response.raise_for_status()
...
class IntentModel:
    ...
    def predict(
        ...
        response = requests.post(
            self.intent_server_endpoint, json=intent_request.dict(), timeout=60
        )
        response.raise_for_status()
